      ...
      <main>
        <section className="section-padding" aria-label={t('title')}>
          <div className="container-main">
              ...
              {pillars.map(({ icon: Icon, title, text }, index) => (
                <Reveal key={title} delay={index * 70}>
                  <article className="border-t border-sand-300 pt-6">
                    ...
                    <p className="mt-3 max-w-xl text-base leading-8 text-sea-900/68">{text}</p>
                  </article>
                </Reveal>
              ))}
            </div>
            ...
            <Reveal>
              <blockquote className="mt-16 border-s-4 border-sun-500 py-3 ps-7 font-display text-3xl font-extrabold leading-tight text-sea-900 sm:text-4xl md:text-5xl">
                {t('tagline')}
              </blockquote>
            </Reveal>
          </div>
