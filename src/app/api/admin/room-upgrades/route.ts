...
// ─────────────────────────────────────────────────────────────────────────────
// Room Upgrades Admin API — migration-safe
// Returns { migrationPending: true } when accommodation_room_upgrades does not
// exist yet, so the admin UI can show a "migration required" notice.
// ─────────────────────────────────────────────────────────────────────────────
...
const upgradeSchema = z.object({
  ...
  extra_price_per_night: z.number().min(0),
...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    if (isMissingTable(error)) return NextResponse.json({ migrationPending: true, upgrades: [] })
...
export async function POST(req: NextRequest) {
  ...
  if (error) {
    if (isMissingTable(error)) return NextResponse.json({ migrationPending: true }, { status: 503 })
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
